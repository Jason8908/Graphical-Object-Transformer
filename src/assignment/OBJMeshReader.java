...
public class OBJMeshReader implements MeshReader {
	...
	public HashSet<Polygon> read(String filename) throws IOException, WrongFileFormatException {
		...
		Pattern vertex = Pattern.compile("\s*v(\s+(-)?[0-9]+(\\.[0-9]+)?){3}\s*");
		Pattern faces = Pattern.compile("\s*f(\s+[0-9]+)+(\s)*");
		ArrayList<ArrayList<String>> data = new ArrayList<ArrayList<String>>();
		// Setting up the reader.
		BufferedReader b = new BufferedReader(new FileReader(filename));
		// Reading all the lines and validating each one.
		String line = b.readLine();
		if (line == null) throw new WrongFileFormatException("OBJ File is formatted incorrectly. Empty file.");
		// If the file does not start with a vertex line.
		Matcher matcher = vertex.matcher(line);
		if (!matcher.matches()) throw new WrongFileFormatException("OBJ File is formatted incorrectly. The first line: " + line + " is not a valid vertex.");
		// Reading all of the vertices.
		ArrayList<String> verticesL = new ArrayList<String>();
		while (matcher.matches() && line != null) {
			// Adding the line to the string.
			verticesL.add(line);
			// Updating the line and the matcher.
			line = b.readLine();
			// If line is empty (file has no faces).
			if (line == null) throw new WrongFileFormatException("OBJ File is formatted incorrectly. There are no face lines in the file.");
			// Updating the matcher
			matcher = vertex.matcher(line);
		}
		// Reading all of the faces.
		matcher = faces.matcher(line);
		ArrayList<String> facesL = new ArrayList<String>();
		while(line != null) {
			// Updating the matcher
			matcher = faces.matcher(line);
			// If the line does not match a vertex line or a face line.
			if (!matcher.matches()) throw new WrongFileFormatException("OBJ File is formatted incorrectly. " + line + " is not a valid face.");
			// Adding the line to the string.
			facesL.add(line);
			// Updating the line and the matcher.
			line = b.readLine();
		...
		for (String line1 : data.get(0)) {
			...
			for (int i = 1; i < seperate.length; i++) coords[i-1] = Double.parseDouble(seperate[i]);
		...
		for (String line1 : data.get(1)) {
			...
			for (int i = 1; i < seperated.length; i++) {
				...
				int index = Integer.parseInt(seperated[i])-1;
				// Getting the vertex and throwing an exception if not present.
				Vertex v;
				try {
					v = vertices.get(index);
				} catch (IndexOutOfBoundsException e) {
					throw new WrongFileFormatException("OBJ File is formatted incorrectly. Index of vertex is out of bounds.");
				}
